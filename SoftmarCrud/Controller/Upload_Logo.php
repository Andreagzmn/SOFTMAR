...
if (!file_exists($directorio)) { 
  ...
  $archivo     = $directorio.basename($_FILES["Imagen_Logo"]["name"]);
  $uploadOk    = 0;
  $extension_archivo = pathinfo($archivo,PATHINFO_EXTENSION);
  
   $archivo = $directorio."logo.".$extension_archivo;

  $check = getimagesize($_FILES["Imagen_Logo"]["tmp_name"]);
      if($check !== false) {
...
// ASIGNAMOS UN LIMITE DE PESO A NUESTRO ARCHIVO ASIGNANDO UN VALOR EN Bytes
  if($_FILES["Imagen_Logo"]["size"] > 7000000){
...
// VALIDAMOS SI $UPLOADOK ESTA EN 1 DE ESA FORMA SE PUEDE SUBIR
  if($uploadOk == 1){
      if (move_uploaded_file($_FILES["Imagen_Logo"]["tmp_name"], $archivo)) {
          echo "El archivo ". basename( $_FILES["Imagen_Logo"]["name"]). " se subio correctamente.  <br>";
      } else {
